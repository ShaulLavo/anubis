	...
	describe('reactivity', () => {
		it('updates when foldedStarts changes', () => {
			createRoot((dispose) => {
				...
				// Initially no folds - access inside reactive context
				expect(mapping.visibleCount()).toBe(10)

				// For testing signal changes, we need to update synchronously
				// by creating a new mapping with the updated state
				const [foldedStarts2] = createSignal<Set<number>>(new Set([2]))
				const mapping2 = createFoldMapping({
					totalLines: () => 10,
					folds: () => folds,
					foldedStarts: foldedStarts2,
				})
				expect(mapping2.visibleCount()).toBe(7)

				// Verify unfold again
				const [foldedStarts3] = createSignal<Set<number>>(new Set<number>())
				const mapping3 = createFoldMapping({
					totalLines: () => 10,
					folds: () => folds,
					foldedStarts: foldedStarts3,
				})
				expect(mapping3.visibleCount()).toBe(10)
