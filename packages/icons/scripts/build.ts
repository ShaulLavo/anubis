...
const toComponentName = (pack: PackConfig, filePath: string) => {
	...
	const packPrefix = pack.shortName.charAt(0).toUpperCase() + pack.shortName.slice(1)
...
const loadIcon = async (pack: PackConfig, filePath: string): Promise<IconDefinition | null> => {
...
const createEsmBlock = (icon: IconDefinition) => `export function ${icon.name}(props) {
\treturn IconTemplate({
...
const createCjsBlock = (icon: IconDefinition) => `module.exports.${icon.name} = function ${icon.name}(props) {
\treturn IconTemplate({
\t\ta: ${JSON.stringify(icon.attribs)},
\t\tc: ${JSON.stringify(icon.contents)}
\t}, props)
...
const createDeclaration = (icon: IconDefinition) => `export declare const ${icon.name}: IconTypes`

const writeAggregateFiles = async (packDir: string, icons: IconDefinition[]) => {
	const esm = [
...
const buildLibArtifacts = async () => {
	...
	await rename(path.join(LIB_PATH, 'esm', 'index.jsx'), path.join(LIB_PATH, 'index.jsx'))
	await rename(path.join(LIB_PATH, 'esm', 'index.d.ts'), path.join(LIB_PATH, 'index.d.ts'))
	await rm(path.join(LIB_PATH, 'esm'), { recursive: true, force: true })

	await rename(path.join(LIB_PATH, 'cjs', 'index.jsx'), path.join(LIB_PATH, 'index.cjs'))
	await rm(path.join(LIB_PATH, 'cjs'), { recursive: true, force: true })

	await writeFile(path.join(LIB_PATH, 'package.json'), JSON.stringify(LIB_PACKAGE_JSON, null, 2))
}
...
const writeDistPackage = async () => {
	...
	await writeFile(path.join(DIST_PATH, 'package.json'), JSON.stringify(distPackage, null, 2))
}
