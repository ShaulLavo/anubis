...
import { createTabIdentity, parseTabIdentity, getTabsForFile } from '../utils/tabIdentity'
...
describe('Tab Closure Specificity Properties', () => {
	...
	it('property: tab closure affects only the specific tab identity', () => {
		fc.assert(
			fc.property(
				fc.record({
					filePath: fc.constantFrom('test.txt', '.system/settings.json', 'binary.exe'),
					viewModes: fc.array(
		...
		fc.assert(
			fc.property(
				fc.record({
					files: fc.array(
						fc.record({
							path: fc.constantFrom('file1.txt', 'file2.js', '.system/settings.json'),
							modes: fc.array(
								fc.constantFrom('editor', 'ui', 'binary'),
				...
				(config) => {
					...
					const otherTabs = allTabs.filter(tab => !firstFileTabs.includes(tab))
