		...
		fc.assert(
			fc.property(
				fc.record({
					...
					modeSequence: fc.array(
						fc.constantFrom('editor', 'binary'),
						{ minLength: 2, maxLength: 5 }
					),
				}),
				(config) => {
					const stats: ParseResult = { contentKind: 'binary' }
					const availableModes = detectAvailableViewModes(config.binaryFile, stats)
		...
		fc.assert(
			fc.property(
				...
				(testCase) => {
					...
					// All binary files should have the same available modes
