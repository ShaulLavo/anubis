					...
					fc.record({
						path: fc.oneof(
							...
							fc
								...
								.map(
									(parts) =>
										'/' + parts.map((p) => p.replace(/[\0\/]/g, '_')).join('/')
								)
						),
						name: fc
					...
					async (testData) => {
						...
						const cachedEntry: CachedDirectoryEntry = {
							...
							children: testData.children.map((child) => ({
								...
								size:
									child.kind === 'file' ? (child.size ?? undefined) : undefined,
								lastModified:
									child.kind === 'file'
						...
						for (let i = 0; i < testData.children.length; i++) {
							...
							if (originalChild.kind === 'file') {
								expect(retrievedChild.size).toBe(
									originalChild.size ?? undefined
								)
								expect(retrievedChild.lastModified).toBe(
									originalChild.lastModified ?? undefined
								)
							} else {
								expect(retrievedChild.isLoaded).toBe(
									originalChild.isLoaded ?? undefined
								)
							}
						}
			...
			await fc.assert(
				fc.asyncProperty(
					...
					async (testData) => {
						...
						const isFresh = await workerCache.isDirectoryFresh(
							testData.path,
							testData.currentMtime
						)
						...
						const isFreshWithoutMtime = await workerCache.isDirectoryFresh(
							testData.path
						)
						expect(isFreshWithoutMtime).toBe(true)

						// Test freshness for non-existent entry
						const nonExistentFresh = await workerCache.isDirectoryFresh(
							'/non-existent-path',
							testData.currentMtime
						)
			...
			await fc.assert(
				fc.asyncProperty(
					...
					async (testData) => {
						...
						expect(cachedResult!.children).toHaveLength(
							scannedData.children.length
						)
						...
						for (let i = 0; i < scannedData.children.length; i++) {
							...
							if (originalChild.kind === 'file') {
								...
								expect(cachedChild.lastModified).toBe(
									originalChild.lastModified ?? undefined
								)
							} else {
				...
				fc.asyncProperty(
					...
					async (scanResults) => {
						...
						for (const [path, originalEntry] of batchEntries) {
							...
							expect(cached!.children).toHaveLength(
								originalEntry.children.length
							)
