...
import { validateKeyFormat, validateSettingDefinition, validateSettingsSchema } from './validateSchema'
...
describe('Schema Validation Properties', () => {
	...
	it('Property 12: Schema Validation - all settings must have required fields', () => {
		fc.assert(
			fc.property(
				fc.record({
					key: fc.array(fc.stringMatching(/^[a-z]+$/), { minLength: 2, maxLength: 3 }).map(parts => parts.join('.')),
					type: fc.constantFrom('boolean', 'string', 'number'),
					default: fc.oneof(fc.boolean(), fc.string(), fc.integer(), fc.constant(null)),
			...
			fc.property(
				...
				(incompleteSetting) => {
					...
					const missingFields = requiredFields.filter(field => 
						!(field in incompleteSetting) || incompleteSetting[field as keyof typeof incompleteSetting] === undefined
					)
					...
					const missingFieldErrors = result.errors.filter(error => error.type === 'missing_field')
			...
			fc.property(
				fc.record({
					key: fc.oneof(
						...
						fc.array(fc.stringMatching(/^[a-z]+$/), { minLength: 2, maxLength: 3 }).map(parts => parts.join('.')),
						// Invalid key
				...
				(setting) => {
					...
					if (isValidKeyFormat) {
						...
						const hasKeyFormatError = result.errors.some(error => error.type === 'invalid_key_format')
