...
function simulateSettingsTabSingleton(
	openSettingsCalls: string[]
): { tabCount: number; activePath: string | null } {
...
describe('SettingsTab', () => {
	...
	it('property: settings tab singleton', () => {
		fc.assert(
			fc.property(
				fc.array(fc.constantFrom('openSettings', 'otherAction'), { minLength: 1, maxLength: 10 }),
				(actions) => {
					// Filter to only openSettings calls
					const openSettingsCalls = actions.filter(action => action === 'openSettings')
		...
		fc.assert(
			fc.property(
				...
				(categories, initialCategory) => {
					...
					const selectedCategory = simulateDefaultCategorySelection(categories, initialCategory)
	...
	it('should handle category selection correctly', () => {
		...
		expect(simulateDefaultCategorySelection(testCategories, 'appearance')).toBe('appearance')
