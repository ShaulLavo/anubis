...
function simulateJSONViewRoundTrip(
	originalSettings: Record<string, unknown>
): { success: boolean; finalSettings: Record<string, unknown> } {
	try {
			...
			fc.property(
				...
				(originalSettings) => {
					...
					for (const key of originalKeys) {
						...
						expect(typeof result.finalSettings[key]).toBe(typeof originalSettings[key])
					}
				}
			),
			{ numRuns: 100 }
		...
		fc.assert(
			fc.property(
				...
				(settingsWithTypes) => {
					...
					expect(result.finalSettings['editor.enabled']).toStrictEqual(properSettings['editor.enabled'])
					expect(result.finalSettings['editor.theme']).toStrictEqual(properSettings['editor.theme'])
					expect(result.finalSettings['editor.fontSize']).toStrictEqual(properSettings['editor.fontSize'])
					expect(result.finalSettings['editor.mode']).toStrictEqual(properSettings['editor.mode'])
				}
			),
			{ numRuns: 100 }
		)
